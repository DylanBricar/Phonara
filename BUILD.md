...
This guide covers how to set up the development environment and build Phonara from source across different platforms.
...
```bash
git clone git@github.com:DylanBricar/Phonara.git
cd Phonara
```
...
The raw binary (`src-tauri/target/release/phonara`) cannot run standalone — it needs Tauri resource files (tray icons, sounds, VAD model) to be co-located at the expected path.
...
```
Bundling Phonara_*_amd64.AppImage
...
~/.cache/tauri/linuxdeploy-x86_64.AppImage --appimage-extract-and-run \
  --appdir Phonara.AppDir --plugin gtk --output appimage
```
