...
pub fn get_icon_path(theme: AppTheme, state: TrayIconState, warning: bool) -> &'static str {
    if warning && state == TrayIconState::Idle {
        return match theme {
            ...
            AppTheme::Colored => "resources/phonara.png",
        };
    }
    match (theme, state) {
        ...
        (AppTheme::Colored, TrayIconState::Idle) => "resources/phonara.png",
...
/// process-constant updater-disable env flags behind
...
fn build_menu(app: &AppHandle, inputs: &MenuInputs) -> tauri::Result<(Menu<tauri::Wry>, String)> {
    ...
    // When update checks are forced off (e.g. PHONARA_DISABLE_UPDATER, set by
...
/// tooltip. Called when the user "relaunches" Phonara while it is already running
...
mod tests {
    use super::{
        get_icon_path, last_transcript_text, load_tray_icon, AppTheme, MenuInputs, TrayDesired,
        TrayIconState,
    };
    ...
    fn build_entry(transcription: &str, post_processed: Option<&str>) -> HistoryEntry {
        HistoryEntry {
            id: 1,
            file_name: "phonara-1.wav".to_string(),
            timestamp: 0,
            saved: false,
    ...
    #[test]
    fn linux_idle_tray_uses_the_bundled_phonara_icon() {
        assert_eq!(
            get_icon_path(AppTheme::Colored, TrayIconState::Idle, false),
            "resources/phonara.png"
        );
        assert_eq!(
            get_icon_path(AppTheme::Colored, TrayIconState::Idle, true),
            "resources/phonara.png"
        );
    }
