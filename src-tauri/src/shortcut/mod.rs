...
//! - `phonara_keys`: Uses the handy-keys library for more control
//!
...
// Note: Commands are accessed via shortcut::phonara_keys:: in lib.rs
...
pub fn init_shortcuts(app: &AppHandle) {
    ...
    match user_settings.keyboard_implementation {
        ...
        KeyboardImplementation::PhonaraKeys => {
            if let Err(e) = phonara_keys::init_shortcuts(app) {
                error!("Failed to initialize phonara-keys shortcuts: {}", e);
                ...
                // Update settings to persist the fallback so we don't retry PhonaraKeys on next launch
...
pub fn register_cancel_shortcut(app: &AppHandle) {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::PhonaraKeys => phonara_keys::register_cancel_shortcut(app),
    }
}
...
pub fn unregister_cancel_shortcut(app: &AppHandle) {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::PhonaraKeys => phonara_keys::unregister_cancel_shortcut(app),
    }
}
...
pub fn register_shortcut(app: &AppHandle, binding: ShortcutBinding) -> Result<(), String> {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::PhonaraKeys => phonara_keys::register_shortcut(app, binding),
    }
}
...
pub fn unregister_shortcut(app: &AppHandle, binding: ShortcutBinding) -> Result<(), String> {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::PhonaraKeys => phonara_keys::unregister_shortcut(app, binding),
    }
}
...
pub fn get_keyboard_implementation(app: AppHandle) -> String {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::PhonaraKeys => "phonara_keys".to_string(),
    }
}
...
fn validate_shortcut_for_implementation(
    ...
    match implementation {
        ...
        KeyboardImplementation::PhonaraKeys => phonara_keys::validate_shortcut(raw),
    }
}
...
fn parse_keyboard_implementation(s: &str) -> KeyboardImplementation {
    match s {
        ...
        "handy_keys" | "phonara_keys" => KeyboardImplementation::PhonaraKeys,
        other => {
            warn!(
...
fn register_all_shortcuts_for_implementation(
    ...
    for (id, default_binding) in &default_bindings {
            ...
            validate_shortcut_for_implementation(&binding.current_binding, implementation)
        {
            ...
            reset_updates.push((id.clone(), binding.clone()));
    ...
    if !reset_updates.is_empty() {
        settings::update_settings(app, |current_settings| {
            for (id, binding) in reset_updates {
                current_settings.bindings.insert(id, binding);
            }
        });
    }
...
/// Initialize PhonaraKeys if not already initialized, with rollback on failure
fn initialize_phonara_keys_with_rollback(app: &AppHandle) -> Result<bool, String> {
    if app.try_state::<phonara_keys::PhonaraKeysState>().is_some() {
        return Ok(false); // Already initialized, caller should continue
    }

    if let Err(e) = phonara_keys::init_shortcuts(app) {
        error!("Failed to initialize PhonaraKeys: {}", e);
        // Rollback to Tauri
        settings::update_settings(app, |settings| {
            settings.keyboard_implementation = KeyboardImplementation::Tauri;
        });
...
/// Persist an accelerator mutation transactionally and make the next model use
/// reload with it. The currently running transcription keeps its engine.
fn update_accelerator_and_reload_next_use(
    app: &AppHandle,
    update: impl FnOnce(&mut settings::AppSettings) -> Result<(), String>,
) -> Result<(), String> {
    settings::try_update_settings(app, update)?;
...
fn apply_transcribe_acceleration_settings(
    ...
    settings.transcribe_gpu_device = normalized_device;
    settings.whisper_use_gpu = accelerator != settings::TranscribeAcceleratorSetting::Cpu;
    Ok(())
}
...
pub fn change_ort_accelerator_setting(
    ...
    update_accelerator_and_reload_next_use(&app, |current| {
        current.ort_accelerator = accelerator;
        Ok(())
    })
}
...
mod tests {
    ...
    #[test]
    fn transcribe_acceleration_normalizes_non_gpu_devices() {
        for accelerator in [
    ...
    #[test]
    fn transcribe_acceleration_requires_a_valid_gpu_device() {
        let mut settings = get_default_settings();
        let original = (
