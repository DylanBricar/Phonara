...
const HTTP_REQUEST_TIMEOUT: Duration = Duration::from_secs(180);
        ...
        if let Ok(env_url) = env::var(CUSTOM_LLM_BASE_URL_ENV) {
            ...
            if !trimmed.is_empty() {
                debug!(
                    ...
                    CUSTOM_LLM_BASE_URL_ENV,
                    sanitized_url_for_log(trimmed)
                );
...
fn build_headers(provider: &PostProcessProvider, api_key: &str) -> Result<HeaderMap, String> {
    ...
    headers.insert(
        REFERER,
        HeaderValue::from_static("https://github.com/DylanBricar/Phonara"),
    );
    headers.insert(
        USER_AGENT,
        HeaderValue::from_static("Phonara/1.0 (+https://github.com/DylanBricar/Phonara)"),
    );
...
async fn fetch_gemini_models(api_key: &str) -> Result<Vec<String>, String> {
    ...
    let client = reqwest::Client::builder()
        ...
        .timeout(HTTP_REQUEST_TIMEOUT)
        .build()
        .map_err(|e| report_reqwest_error("Failed to build Gemini HTTP client", &e))?;
    let response = client
        .get(url)
    ...
    let status = response.status();
    if !status.is_success() {
        return Err(format!("Gemini model list request failed ({})", status));
    }
