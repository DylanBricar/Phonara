...
fn init_gtk_layer_shell(overlay_window: &tauri::webview::WebviewWindow) -> bool {
    if utils::env_flag_enabled("PHONARA_NO_GTK_LAYER_SHELL")
        || utils::env_flag_enabled("HANDY_NO_GTK_LAYER_SHELL")
    {
        debug!(
            "Skipping GTK layer shell init (PHONARA_NO_GTK_LAYER_SHELL or legacy HANDY_NO_GTK_LAYER_SHELL is enabled)"
        );
        return false;
    }
...
fn get_monitor_with_cursor(app_handle: &AppHandle) -> Option<tauri::Monitor> {
    if let Some(mouse_location) = input::get_cursor_position(app_handle) {
        ...
        if let Ok(monitors) = app_handle.available_monitors() {
            for monitor in monitors {
                // GetCursorPos and Tauri's Windows monitor bounds are both in
                // physical pixels, including on mixed-DPI desktops.
        ...
        #[cfg(not(target_os = "windows"))]
        if let Some(monitor) = get_monitor_containing_logical_point(
            app_handle,
...
fn is_mouse_within_monitor(
    ...
    is_point_within_monitor(
        (mouse_pos.0 as f64, mouse_pos.1 as f64),
        monitor_pos,
        monitor_size,
    )
}
