...
fn create_audio_recorder(
    ...
    let recorder = AudioRecorder::new()
        ...
        .with_pause_flag(Arc::clone(&is_paused))
        ...
        .with_level_callback({
            ...
            move |levels| {
                if is_paused.load(Ordering::Relaxed) {
                    utils::emit_levels(&app_handle, &vec![0.0; levels.len()]);
                } else {
...
impl AudioRecordingManager {
    ...
    /// Snapshot the mute-session generation before scheduling delayed work.
    ...
    pub fn apply_mute(&self, generation: u64) {
        ...
        if mute_guard.generation != generation || mute_guard.did_mute {
            return;
        }
        if *is_open {
    ...
    pub fn remove_mute(&self) {
        let mut mute_guard = self.mute_state.lock().unwrap();
        // Invalidate any delayed readiness/chime worker from this session.
    ...
    pub fn toggle_pause(&self) -> bool {
        let was_paused = self.is_paused.fetch_xor(true, Ordering::Relaxed);
