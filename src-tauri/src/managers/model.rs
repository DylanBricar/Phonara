...
impl ModelManager {
            ...
            if matches!(model.source, ModelSource::HuggingFace { .. }) {
                ...
                let is_alternate = matches!(
                    ...
                    ModelSource::HuggingFace { repo_id, .. }
                        if Self::is_catalog_alternate_quant(repo_id, &model.filename)
                );
                if !model.is_downloaded && is_alternate {
    ...
    async fn download_hf_model(
        ...
        let hf_error = loop {
            ...
            if crate::catalog::is_catalog_model(&model_id) {
                // Catalog repositories are public. A stale ambient token must
                // not turn their downloads into opaque authentication errors;
                // discovered private repositories keep their credentials.
    ...
    pub fn delete_model(&self, model_id: &str) -> Result<()> {
        ...
        if let ModelSource::HuggingFace { repo_id, revision } = &model_info.source {
            ...
            if is_alternate_quant {
                // Alternate entries own only their quant, not the entire repo.
                ...
                // Default catalog entries own the cached repo (blobs, refs and
                // snapshots), matching the established hard-delete behavior.
            ...
            // Mirror downloads and manual catalog drop-ins live beside the
            // app's URL models. Remove both the completed file and resumable
            // partial, while retaining all failures for an actionable error.
            for path in [
            ...
            // Alternate-quant entries are discovery-created, so deleting one
            // removes its row instead of leaving a non-downloadable ghost.
            if is_alternate_quant {
    ...
    pub fn get_model_path(&self, model_id: &str) -> Result<PathBuf> {
        ...
        if matches!(model_info.source, ModelSource::HuggingFace { .. }) {
            let path = self.local_artifact(&model_info).ok_or_else(|| {
                anyhow::anyhow!(
            ...
            // A verified complete models-dir copy wins over a stale partial
            // left by an interrupted mirror attempt.
