...
impl TranscriptionManager {
    ...
    /// Shared handle used by the audio recorder to feed real-time frames.
    ...
    fn run_stream_worker(&self, rx: mpsc::Receiver<StreamCmd>, worker_id: u64) {
        ...
        let mut engine = match self.lock_engine().take() {
            ...
            None => {
                info!(
                    "Live preview: model '{}' was unloaded before streaming could begin; falling back to batch transcription",
                    model_id
                );
                let _ = self.active_engine_lease.compare_exchange(
                    worker_id,
        ...
        let (supports_streaming, supports_translate, languages) = match &engine {
            LoadedEngine::TranscribeCpp(session) => {
                ...
                info!(
                    "Live preview: model '{}' arch='{}' variant='{}' supports_streaming={} supports_translate={} languages={:?}",
                    model_id,
                    model.arch(),
                    model.variant(),
            ...
            _ => {
                info!(
                    "Live preview: model '{}' is not a transcribe-cpp model; streaming is unavailable, using batch transcription",
                    model_id
                );
                (false, false, Vec::new())
            }
    ...
    /// Abandon any pending stream route without producing text.
    ...
    /// Emit a working-phase event to the streaming overlay.
    ...
    pub fn transcribe(&self, audio: Vec<f32>) -> Result<String> {
        ...
        if std::env::var("PHONARA_FORCE_TRANSCRIPTION_FAILURE").is_ok() {
            return Err(anyhow::anyhow!(
                "Simulated transcription failure (PHONARA_FORCE_TRANSCRIPTION_FAILURE)"
            ));
        }
        ...
        let result = {
            ...
            let transcribe_result = catch_unwind(AssertUnwindSafe(|| -> Result<String> {
                match &mut engine {
                    LoadedEngine::TranscribeCpp(session) => {
                        let initial_prompt = if model_is_whisper {
