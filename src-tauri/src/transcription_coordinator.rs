...
impl CoordinatorState {
    ...
    fn on_select_action(&mut self, key: u8) {
        ...
        if self.selected_action == Some(key) {
            self.selected_action = None;
            debug!("Action {key} deselected during recording");
        } else {
            self.selected_action = Some(key);
            debug!("Action {key} selected during recording");
        }
    }
    ...
    fn begin_processing(&mut self, binding_id: String, hotkey_string: String) -> Effect {
        ...
        Effect::Stop {
            binding_id,
            hotkey_string,
            selected_action: self.selected_action.take(),
        }
    }
}
...
pub fn parse_action_key(id: &str) -> Option<u8> {
    id.strip_prefix("action_").and_then(|key| key.parse().ok())
}
...
fn store_active_action(app: &AppHandle, selected_action: Option<u8>) {
    let Some(state) = app.try_state::<ActiveActionState>() else {
        return;
    };
    match state.0.lock() {
        Ok(mut active) => *active = selected_action,
        Err(poisoned) => {
...
fn run_effect(app: &AppHandle, state: &mut CoordinatorState, effect: Effect) {
    match effect {
        ...
        Effect::Stop {
            ...
            store_active_action(app, selected_action);
...
mod tests {
    ...
    fn action_binding_keys_are_parsed_strictly() {
        ...
        assert_eq!(parse_action_key("transcribe"), None);
    }

    #[test]
    fn recording_action_selection_toggles_and_is_attached_to_stop() {
        let mut state = CoordinatorState::new();
        let now = Instant::now();
        state.begin_recording("transcribe".into(), "Alt+Space".into(), now, true);

        state.on_select_action(2);
        state.on_select_action(3);
        state.on_select_action(3);
        assert_eq!(state.selected_action, None);

        state.on_select_action(2);
        assert_eq!(
            state.begin_processing("transcribe".into(), "Alt+Space".into()),
            Effect::Stop {
                binding_id: "transcribe".into(),
                hotkey_string: "Alt+Space".into(),
    ...
    #[test]
    fn action_selection_is_ignored_while_idle() {
        let mut state = CoordinatorState::new();
        state.on_select_action(4);
        assert_eq!(state.selected_action, None);
    }
