...
type ApiExamplesProps = {
  ...
  highlightedExamples?: Record<string, string>;
  highlightedResults?: Record<string, string>;
  className?: string;
};
...
export function DocsApiExamples({
  ...
  const [requestExample, setRequestExample] = useQueryState(
    "request",
    parseAsString,
  );
  const [responseExample, setResponseExample] = useQueryState(
    "response",
    parseAsString,
  );
  const exampleKeys = examples ? Object.keys(examples) : [];
  const resultKeys = results ? Object.keys(results) : [];
  const activeRequestExample =
    requestExample && exampleKeys.includes(requestExample)
      ? requestExample
      : exampleKeys[0];
  const activeResponseExample =
    responseExample && resultKeys.includes(responseExample)
      ? responseExample
      : resultKeys[0];
    ...
    <div className={cn("flex flex-col gap-6", className)}>
      ...
      {examples && exampleKeys.length > 0 && (
        <div className="flex flex-col gap-2">
          ...
          <Tabs
            value={activeRequestExample}
            onValueChange={(value) => {
              void setRequestExample(value === exampleKeys[0] ? null : value);
            }}
            className="w-full"
          >
            <TabsList
            ...
            {exampleKeys.map((key) => (
              <TabsContent key={key} value={key} className="mt-2">
                {highlightedExamples?.[key] ? (
                  <div
                    className="docs-shiki overflow-x-auto rounded-lg text-xs [&_pre]:m-0 [&_pre]:rounded-lg [&_pre]:p-4"
                    dangerouslySetInnerHTML={{
                      __html: highlightedExamples[key],
                    }}
                  />
                ) : (
      ...
      {results && resultKeys.length > 0 && (
        <div className="flex flex-col gap-2">
          ...
          <Tabs
            value={activeResponseExample}
            onValueChange={(value) => {
              void setResponseExample(value === resultKeys[0] ? null : value);
            }}
            className="w-full"
          >
            <TabsList
            ...
            {resultKeys.map((key) => (
              <TabsContent key={key} value={key} className="mt-2">
                {highlightedResults?.[key] ? (
                  <div
                    className="docs-shiki overflow-x-auto rounded-lg text-xs [&_pre]:m-0 [&_pre]:rounded-lg [&_pre]:p-4"
                    dangerouslySetInnerHTML={{
                      __html: highlightedResults[key],
                    }}
                  />
                ) : (
                  <pre className="bg-muted text-foreground overflow-x-auto rounded-lg p-4 text-xs">
