...
examples:
  ...
  javascript: |
    const response = await fetch('https://nowstack.melvynx.dev/api/v1/testimonials', {
      headers: {
...
```bash
curl -H "Authorization: Bearer YOUR_API_TOKEN" \
  "https://nowstack.melvynx.dev/api/v1/testimonials?status=approved&type=video"
```
...
```bash
curl -H "Authorization: Bearer YOUR_API_TOKEN" \
  "https://nowstack.melvynx.dev/api/v1/testimonials?search=amazing%20product"
```
...
```bash
curl -H "Authorization: Bearer YOUR_API_TOKEN" \
  "https://nowstack.melvynx.dev/api/v1/testimonials?rating=4&sort=rating&order=desc"
```
