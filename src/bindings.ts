...
async changeTranscribeAccelerationSetting(accelerator: TranscribeAcceleratorSetting, device: string | null) : Promise<Result<null, string>> {
    try {
...
async hideMainWindowCommand() : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("hide_main_window_command") };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
